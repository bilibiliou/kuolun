const express      = require('express'),
      ...
      isHttps      = process.env.IS_SSL || false,
      ImageStorage = join(__dirname,'/database/img-storage'),
      {
        PublishNew,
        PublishFeedBack,
        getAllDataes,
...
if (!isHttps) {
  http.createServer(app).listen(port,()=>console.log(`正在监听${port}端口`));
} else {
  let options = {
    key: fs.readFileSync(resolve(__dirname, './certificates/key.pem')),
    cert: fs.readFileSync(resolve(__dirname, './certificates/cert.pem'))
  }
  https.createServer(options, app).listen(port,()=>console.log(`正在监听${port}端口`));
}
...
//         url = join(req.originalUrl,'/database/img-storage/',key);
...
app.post('/saveData', (req,res) => {
    ...
    // 防xss
    oData.publish_content = xss(oData.publish_content);
